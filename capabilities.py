...
from .exceptions import CapabilityError, KeiSDKError
...
class MCPIntegration:
    ...
    async def load_mcp_profile(self, capability_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        # Prüfe Cache
        if (time.time() - self._last_cache_update < self._cache_ttl and
            capability_name in self._mcp_profiles_cache):
        ...
        try:
            ...
            raise CapabilityError(f"MCP-Profil für '{capability_name}' konnte nicht geladen werden: {e}") from e
        ...
        try:
            ...
            for method_name, method_spec in capability_profile.methods.items():
                ...
                if not self._validate_schema_compatibility(
                    method_spec.get("input_schema"),
                    mcp_method.get("input_schema")
                ):
                    return False
                
                # Validiere Output-Schema
                if not self._validate_schema_compatibility(
                    method_spec.get("output_schema"),
                    mcp_method.get("output_schema")
                ):
                    return False
...
class CapabilityNegotiation:
    ...
    def _check_version_compatibility(self, client_version: str, server_version: str) -> CompatibilityLevel:
        ...
        try:
            ...
            if (client_ver.major == server_ver.major and
                client_ver.minor <= server_ver.minor):
...
class CapabilityVersioning:
    ...
    def _get_compatibility(
        ...
        if (capability_name in self._compatibility_matrix and
            cache_key in self._compatibility_matrix[capability_name]):
...
class CapabilityManager:
    ...
    async def register_capability(
        ...
        if profile.mcp_profile_url or profile.mcp_schema:
            ...
            if mcp_profile:
                is_valid = await self.mcp_integration.validate_capability_interface(profile, mcp_profile)
                
                if not is_valid:
                    raise CapabilityError(f"Capability '{profile.name}' ist nicht MCP-kompatibel")
    ...
    async def _advertise_capabilities(self) -> None:
        ...
        try:
            ...
            raise CapabilityError(f"Capability-Advertisement fehlgeschlagen: {e}") from e

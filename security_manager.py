...
class SecurityManager:
    ...
    async def _fetch_oidc_token(self) -> Dict[str, Any]:
        ...
        """
        if not all([self.config.oidc_issuer, self.config.oidc_client_id, self.config.oidc_client_secret]):
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise SecurityError(f"OIDC Token-Request fehlgeschlagen: {e.response.status_code}") from e
    ...
    async def _token_refresh_loop(self) -> None:
        ...
        while True:
            try:
                ...
                _logger.error(
                    "Fehler bei automatischer Token-Erneuerung",
                    extra={"error": str(e)}
                )
    ...
    def get_security_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "refresh_task_active": self._token_refresh_task is not None and not self._token_refresh_task.done()
        }

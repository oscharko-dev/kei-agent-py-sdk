...
from .exceptions import DiscoveryError, CommunicationError
...
class HealthMonitor:
    ...
    async def _check_instance_health(self, instance: AgentInstance) -> None:
        ...
        try:
            ...
            self._recovery_counts[instance_id] = self._recovery_counts.get(instance_id, 0) + 1
...
class ServiceDiscovery:
    ...
    def __init__(self, client: KeiAgentClient, strategy: DiscoveryStrategy = DiscoveryStrategy.REGISTRY_BASED):
        self.client = client
        self.strategy = strategy
...
class AgentDiscoveryClient:
    ...
    async def find_agents_by_capability(self, capability: str, max_results: int = 10) -> List[Agent]:

...
class ConfigAPI:
    ...
    async def update_config_value_handler(self, request: Request) -> Response:
        ...
        try:
            ...
            # Apply update
            success = await self.config_manager.update_config(updates, user_id=user_id, reason=reason)
    ...
    async def validate_config_handler(self, request: Request) -> Response:
        ...
        try:
            validation_info = {
                ...
                "field_types": {k: v.__name__ for k, v in self.config_manager.validator.field_types.items()},
    ...
    async def websocket_config_handler(self, request: Request) -> WebSocketResponse:
        ...
        logger.info(f"Configuration WebSocket connection established. Total: {len(self.websocket_connections)}")
        ...
        try:
            ...
            logger.info(f"Configuration WebSocket connection closed. Remaining: {len(self.websocket_connections)}")
...
def initialize_config_api(config_manager: Optional[ConfigManager] = None, require_auth: bool = True) -> ConfigAPI:

...
def run_command(cmd: List[str], description: str, check: bool = True) -> subprocess.CompletedProcess:
    ...
    try:
        result = subprocess.run(cmd, check=check, capture_output=True, text=True, cwd=BASE_DIR)
...
def run_quality_checks() -> bool:
    ...
    checks = [
        ...
        (["python", "-m", "ruff", "format", "--check", "kei_agent/"], "Ruff Formatting Check"),
        (["python", "-m", "mypy", "kei_agent/"], "MyPy Type Checking"),
        (["python", "-m", "bandit", "-r", "kei_agent/", "-f", "json", "-o", "bandit-report.json"], "Security Scan"),
    ]
...
def run_tests() -> bool:
    ...
    test_commands = [
        ...
        (["python", "-m", "pytest", "tests/", "--cov=kei_agent", "--cov-report=xml", "--cov-fail-under=85"], "Coverage Tests"),
    ]
...
def publish_to_testpypi():
    ...
    result = run_command(
        ["python", "-m", "twine", "upload", "--repository", "testpypi"] + [str(f) for f in dist_files],
        "TestPyPI Upload",
        check=False
    )
...
def main():
    ...
    parser.add_argument("--skip-quality", action="store_true", help="Überspringe Qualitätsprüfungen")
    parser.add_argument("--skip-tests", action="store_true", help="Überspringe Tests")
    parser.add_argument("--build-only", action="store_true", help="Nur Build, keine Veröffentlichung")
    parser.add_argument("--publish-test", action="store_true", help="Auf TestPyPI veröffentlichen")
    parser.add_argument("--publish-prod", action="store_true", help="Auf PyPI veröffentlichen")

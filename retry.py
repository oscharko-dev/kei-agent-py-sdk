...
class CircuitBreakerConfig:
    ...
    # Callbacks
    on_state_change: Optional[Callable[[CircuitBreakerState, CircuitBreakerState], Awaitable[None]]] = None
...
class CircuitBreaker:
    ...
    def _should_open(self) -> bool:
        ...
        if len(self._call_history) >= self.config.minimum_throughput:
            failure_rate = sum(1 for success in self._call_history if not success) / len(self._call_history)
...
class RetryManager:
    ...
    def get_circuit_breaker(self, name: str) -> CircuitBreaker:
        ...
        if name not in self._circuit_breakers:
            ...
            _logger.info("Circuit Breaker initialisiert: %s (threshold=%s, recovery=%ss, half_open=%s)",
                        name, cb_config.failure_threshold, cb_config.recovery_timeout, cb_config.half_open_max_calls)
        ...
        **kwargs
            ...
            for attempt in range(policy.max_attempts):
                try:
                    ...
                    _logger.debug(
                        ...
                        attempt + 2, policy.max_attempts, proto, op, delay, repr(e)
                    )
        ...
        for attempt in range(policy.max_attempts):
            try:
                ...
                # Prüfe ob Retry durchgeführt werden soll
                if not policy.should_retry(e, attempt):
                    _logger.debug("Kein Retry nach Versuch %d: %s", attempt + 1, repr(e))
                    break

                # Letzter Versuch - kein weiterer Retry
                if attempt >= policy.max_attempts - 1:
                    _logger.debug("Letzter Versuch %d erreicht, kein weiterer Retry", attempt + 1)
                    break

                # Warte vor nächstem Versuch
                delay = policy.calculate_delay(attempt)
                _logger.debug("Retry-Versuch %d/%d (Delay=%.3fs): %s", attempt + 2, policy.max_attempts, delay, repr(e))
                await asyncio.sleep(delay)

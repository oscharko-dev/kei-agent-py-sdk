...
class SpanBuilder:
    ...
    def with_attribute(self, key: str, value: Union[str, int, float, bool]) -> SpanBuilder:
    ...
    def with_attributes(self, attributes: Dict[str, Union[str, int, float, bool]]) -> SpanBuilder:
    ...
    def with_link(self, span_context: trace.SpanContext, attributes: Optional[Dict[str, Any]] = None) -> SpanBuilder:
...
class TracingManager:
    ...
    def _initialize_tracing(self) -> None:
        ...
        try:
            ...
            if self.config.jaeger_endpoint:
                jaeger_exporter = TracingExporter("jaeger", 
                    collector_endpoint=self.config.jaeger_endpoint)
                self._tracer_provider.add_span_processor(jaeger_exporter.get_span_processor())
            else:
                # Fallback: Console-Exporter
                console_exporter = TracingExporter("console")
                self._tracer_provider.add_span_processor(console_exporter.get_span_processor())
    ...
    def record_performance_metrics(self, metrics: PerformanceMetrics) -> None:
        ...
        if self._duration_histogram:
            self._duration_histogram.record(metrics.duration_ms, {
                "operation": metrics.operation_name,
                "status": metrics.status
            })

...
class ProtocolSelector:
    ...
    def _get_fallback_protocol(self) -> Optional[ProtocolType]:
        ...
        available_protocols = [
            protocol for protocol in self._protocol_priorities.keys()
    ...
    def analyze_operation_requirements(self, operation: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if any(pattern in operation_lower for pattern in ["stream", "live", "realtime", "subscribe"]):
            analysis["requirements"]["streaming"] = True
            analysis["requirements"]["realtime"] = True
            
        # Asynchrone Anforderungen
        if any(pattern in operation_lower for pattern in ["async", "background", "queue", "publish"]):
            analysis["requirements"]["async"] = True
            
        # Tool-Anforderungen
        if any(pattern in operation_lower for pattern in ["tool", "resource", "prompt", "discover"]):

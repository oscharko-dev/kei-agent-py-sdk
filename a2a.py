...
from .models import Agent, AgentInstance
...
class A2AClient:
    ...
    def enable_service_discovery(
        ...
        self._load_balancer = LoadBalancer(
            ...
            health_tracker=self._instance_health
        )
    ...
    async def send_message(
        ...
        **kwargs
        ...
        if self._tracer:
            with self._tracer.start_as_current_span(f"a2a.send_message.{target_agent}") as span:
    ...
    async def _send_message_with_discovery(
        ...
        try:
            ...
            if self._service_discovery:
                ...
                if not instances:
                    raise AgentNotFoundError(f"Agent '{message.to_agent}' nicht gefunden")
            ...
            response = await self._send_with_failover(message, target_instance, protocol, timeout)
            ...
            if isinstance(e, (AgentNotFoundError, CommunicationError)):
                ...
                raise CommunicationError(f"A2A-Kommunikation fehlgeschlagen: {e}") from e
        ...
        for attempt in range(self.failover_config.max_retries + 1):
            try:
                ...
                if protocol == CommunicationProtocol.HTTP:
                    response = await self._send_http_message(message, target_instance, timeout)
                elif protocol == CommunicationProtocol.WEBSOCKET:
                    response = await self._send_websocket_message(message, target_instance, timeout)
                elif protocol == CommunicationProtocol.MESSAGE_BUS:
                    response = await self._send_message_bus_message(message, target_instance, timeout)
                else:
                    raise CommunicationError(f"Protokoll '{protocol}' nicht unterstützt")
                ...
                if self.failover_config.on_instance_failed:
                    await self.failover_config.on_instance_failed(target_instance.instance_id)
                ...
                if self._service_discovery:
                    alternative_instances = await self._discover_agent_instances(message.to_agent)
                    available_instances = [
                        inst for inst in alternative_instances
                        if inst.instance_id != target_instance.instance_id and
                           inst.instance_id not in self._failed_instances
                    ]
                    
                    if available_instances:
                        if self._load_balancer:
                            target_instance = self._load_balancer.select_instance(available_instances)
                        else:
    ...
    async def _get_websocket_connection(
        ...
        if instance_id not in self._websocket_connections:
            ...
            ws_url = target_instance.endpoint.replace("http://", "ws://").replace("https://", "wss://")
    ...
    def _mark_instance_failed(self, instance_id: str) -> None:
        ...
        if instance_id not in self._instance_health:
            ...
            self._instance_health[instance_id].update({
                "status": "unhealthy",
                "last_failure": time.time(),
                "consecutive_failures": self._instance_health[instance_id].get("consecutive_failures", 0) + 1
            })

            ...
            try:
                ...
                async with session.request(
                    ...
                    if response.status >= 400:
                        ...
                        if (response.status in self.config.retry.retry_on_status_codes and 
                            ...
                            if self.config.on_retry_attempt:
                                await self.config.on_retry_attempt(attempt + 1, 
                        ...
                        if response.status == 404:
                            raise AgentNotFoundError(f"Agent nicht gefunden: {error_text}")
                        else:
                ...
                if (any(isinstance(e, exc_type) for exc_type in self.config.retry.retry_on_exceptions) and
                    attempt < retry_policy.max_attempts - 1):
                ...
                if (isinstance(e, (aiohttp.ClientConnectionError, aiohttp.ServerDisconnectedError)) and
                    self.config.on_connection_error):

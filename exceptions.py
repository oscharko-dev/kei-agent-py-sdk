...
class RetryExhaustedError(KeiSDKError):
    ...
    def __init__(self, attempts: int, last_error: Optional[Exception] = None, **kwargs: Any) -> None:
...
class CircuitBreakerOpenError(KeiSDKError):
    ...
    def __init__(self, service_name: str, **kwargs: Any) -> None:
        ...
        super().__init__(message, error_code="CIRCUIT_BREAKER_OPEN", service_name=service_name, **kwargs)
...
class AuthenticationError(KeiSDKError):
    ...
    def __init__(self, message: str = "Authentifizierung fehlgeschlagen", **kwargs: Any) -> None:
...
class TimeoutError(KeiSDKError):
    ...
    def __init__(self, timeout_seconds: float, operation: Optional[str] = None, **kwargs: Any) -> None:
        message = f"Timeout nach {timeout_seconds}s"
        if operation:
            message += f" für Operation: {operation}"
        super().__init__(message, error_code="TIMEOUT_ERROR", timeout_seconds=timeout_seconds, **kwargs)
...
class SecurityError(KeiSDKError):
    ...
    def __init__(self, message: str, security_context: Optional[str] = None, **kwargs: Any) -> None:
        super().__init__(message, error_code="SECURITY_ERROR", security_context=security_context, **kwargs)
